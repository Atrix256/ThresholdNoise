...
Threshold::Context* m_Threshold = nullptr;
...
int main(int, char**)
{
    ...
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Threshold", WS_OVERLAPPEDWINDOW, 100, 100, windowSize.right - windowSize.left, windowSize.bottom - windowSize.top, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    Threshold::Context::LogFn = &LogFunction;
    Threshold::Context::PerfEventBeginFn = &PerfEventBeginFn;
    Threshold::Context::PerfEventEndFn = &PerfEventEndFn;
    m_Threshold = Threshold::CreateContext(g_pd3dDevice);
    if (!m_Threshold)
    {
        printf("Could not create m_Threshold context");
        return 1;
    }
    // Gigi Modification End
    ...
    while (!done)
    {
        ...
        if (m_Threshold)
        {
            ...
            Threshold::MakeUI(m_Threshold, g_pd3dCommandQueue);
        ...
        // Gigi Modification Begin - OnNewFrame and Execute
        {
            ...
            if (firstPreExecute)
            {
                ...
                // change to #if 0 to load the threshold map image instead
                #if 1
                    ...
                    m_Threshold->m_input.texture_ThresholdMap_format = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
            ...
            if (m_Threshold->m_output.texture_Color)
                CopyTextureToTexture(g_pd3dCommandList, m_Threshold->m_output.texture_Color, m_Threshold->m_output.c_texture_Color_endingState, g_mainRenderTargetResource[backBufferIdx], D3D12_RESOURCE_STATE_RENDER_TARGET);
        }
        // Gigi Modification End
    ...
    if (m_Threshold)
    {
        Threshold::DestroyContext(m_Threshold);
        m_Threshold = nullptr;
    }
    // Gigi Modification End
