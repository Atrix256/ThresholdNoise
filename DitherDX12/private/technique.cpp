...
namespace Dither
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Dither_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Dither_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Dither
        {
            ...
            // _DitherCB
            ranges[14].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
            ranges[14].NumDescriptors = 1;
            ranges[14].BaseShaderRegister = 0;
            ranges[14].RegisterSpace = 0;
            ranges[14].OffsetInDescriptorsFromTableStart = 14;

            if(!DX12Utils::MakeRootSig(device, ranges, 15, samplers, 0, &ContextInternal::computeShader_Dither_rootSig, (c_debugNames ? L"Dither" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/DitherCS.hlsl", "csmain", "cs_6_1", defines,
               ContextInternal::computeShader_Dither_rootSig, &ContextInternal::computeShader_Dither_pso, c_debugShaders, (c_debugNames ? L"Dither" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Dither_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Dither_pso);
            ContextInternal::computeShader_Dither_pso = nullptr;
        }

        if(ContextInternal::computeShader_Dither_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Dither_rootSig);
            ContextInternal::computeShader_Dither_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Dither
    ...
    Context::~Context()
    {
        ...
        // _DitherCB
        if (m_internal.constantBuffer__DitherCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__DitherCB);
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Dither", commandList, 22);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Dither Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (3+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Dither Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        // Set variables
        context->m_input.variable_BitsPerColorChannel = max(context->m_input.variable_BitsPerColorChannel, 1);
        context->m_input.variable_BitsPerColorChannel = min(context->m_input.variable_BitsPerColorChannel, 8);
        ...
        {
            Context::LogFn(LogLevel::Error, "Dither: Imported texture \"Input\" is null.\n");
            return;
        }
        ...
        // Shader Constants: _DitherCB
        {
            context->m_internal.constantBuffer__DitherCB_cpu.Animate1 = context->m_input.variable_Animate1;
            context->m_internal.constantBuffer__DitherCB_cpu.Animate2 = context->m_input.variable_Animate2;
            context->m_internal.constantBuffer__DitherCB_cpu.Animate3 = context->m_input.variable_Animate3;
            context->m_internal.constantBuffer__DitherCB_cpu.Animate4 = context->m_input.variable_Animate4;
            context->m_internal.constantBuffer__DitherCB_cpu.BitsPerColorChannel = context->m_input.variable_BitsPerColorChannel;
            context->m_internal.constantBuffer__DitherCB_cpu.ExtendNoise1 = context->m_input.variable_ExtendNoise1;
            context->m_internal.constantBuffer__DitherCB_cpu.ExtendNoise2 = context->m_input.variable_ExtendNoise2;
            context->m_internal.constantBuffer__DitherCB_cpu.ExtendNoise3 = context->m_input.variable_ExtendNoise3;
            context->m_internal.constantBuffer__DitherCB_cpu.ExtendNoise4 = context->m_input.variable_ExtendNoise4;
            context->m_internal.constantBuffer__DitherCB_cpu.FrameIndex = context->m_internal.variable_FrameIndex;
            context->m_internal.constantBuffer__DitherCB_cpu.NoiseType1 = (int)context->m_input.variable_NoiseType1;
            context->m_internal.constantBuffer__DitherCB_cpu.NoiseType2 = (int)context->m_input.variable_NoiseType2;
            context->m_internal.constantBuffer__DitherCB_cpu.NoiseType3 = (int)context->m_input.variable_NoiseType3;
            context->m_internal.constantBuffer__DitherCB_cpu.NoiseType4 = (int)context->m_input.variable_NoiseType4;
            context->m_internal.constantBuffer__DitherCB_cpu.SubtractiveDither1 = context->m_input.variable_SubtractiveDither1;
            context->m_internal.constantBuffer__DitherCB_cpu.SubtractiveDither2 = context->m_input.variable_SubtractiveDither2;
            context->m_internal.constantBuffer__DitherCB_cpu.SubtractiveDither3 = context->m_input.variable_SubtractiveDither3;
            context->m_internal.constantBuffer__DitherCB_cpu.SubtractiveDither4 = context->m_input.variable_SubtractiveDither4;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__DitherCB, context->m_internal.constantBuffer__DitherCB_cpu, Context::LogFn);
        }
        ...
        // Compute Shader: Dither
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Dither", commandList, 0);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Dither_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Dither_pso);
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_internal.constantBuffer__DitherCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0 }
            };

            D3D12_GPU_DESCRIPTOR_HANDLE descriptorTable = GetDescriptorTable(device, s_srvHeap, descriptors, 15, Context::LogFn);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Dither";
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _DitherCB
        if (m_internal.constantBuffer__DitherCB == nullptr)
        {
            dirty = true;
