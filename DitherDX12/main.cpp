...
Dither::Context* m_Dither = nullptr;
...
int main(int, char**)
{
    ...
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Dither", WS_OVERLAPPEDWINDOW, 100, 100, windowSize.right - windowSize.left, windowSize.bottom - windowSize.top, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    Dither::Context::LogFn = &LogFunction;
    Dither::Context::PerfEventBeginFn = &PerfEventBeginFn;
    Dither::Context::PerfEventEndFn = &PerfEventEndFn;
    m_Dither = Dither::CreateContext(g_pd3dDevice);
    if (!m_Dither)
    {
        printf("Could not create m_Dither context");
        return 1;
    }
    // Gigi Modification End
    ...
    while (!done)
    {
        ...
        {
            ImGui::Begin("Dither");
            Dither::MakeUI(m_Dither, g_pd3dCommandQueue);
    ...
    if (m_Dither)
    {
        Dither::DestroyContext(m_Dither);
        m_Dither = nullptr;
    }
    // Gigi Modification End
